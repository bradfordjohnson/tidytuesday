...
color_pal <- c(colors_json$nord$frost$nord9, colors_json$nord$snow_storm$nord4)

caption <- create_caption(2023, 35, background_color, font = font_1)

title <- "<span>Fair Use Analysis: Top Categories and Court Findings</span>"

subtitle <- "<span>Fair use found vs.<span style='color:#81A1C1;'> fair use not found </span></span>"
...
fair_use_cases$categories <- str_to_title(str_remove(fair_use_cases$categories, ";.*")) # remove everything after ;
...
fair_use_cases %>%
    ...
    count() %>%
    mutate(n = case_when(fair_use_found == TRUE ~ n * 1, fair_use_found == FALSE ~ n * -1)) %>%
    mutate(lab_y = case_when(n > 0 ~ 2, n < 0 ~ -2)) %>%
    filter(categories %in% target_categories) %>%
    ggplot(aes(x = reorder(categories, n), y = n, fill = fair_use_found)) +
    geom_col() +
    scale_fill_manual(values = color_pal) +
    geom_text(
        aes(label = abs(n), y = lab_y),
        vjust = 0,
        color = label_color,
        size = 5
    ) +
