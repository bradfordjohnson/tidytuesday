...
p1 <- dozen_eggs |>
  ...
  geom_text(aes(x = x, y = y - .22, label = substr(observed_month, 0, 7)),
    size = 10, color = dozen_eggs$color,
    family = font_1
  ) +
  geom_text(aes(x = x, y = y, label = paste0(round(growth, 1), "%")),
    size = 18, color = dozen_eggs$color,
    family = font_1
  ) +
