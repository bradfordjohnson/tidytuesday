...
ggplot(df_allergies, aes(x = fct_rev(active_substance), y = x)) +
  ...
  theme(
    ...
    plot.title = element_text(
      color = text_color_2, family = font1,
      size = 80, margin = margin(3, 0, 0, 0, "mm")
    ),
    plot.subtitle = ggtext::element_textbox_simple(
      color = text_color_1, family = font2,
      size = 45, margin = margin(4, 0, 4, 0, "mm")
    ),
    axis.text.x = element_text(
      color = c(text_color_3, text_color_1, text_color_1, text_color_1, text_color_1), family = font2,
